...
class IronroadEnv(gym.Env):
    ...
    def _send_gripper_command(self, pos: float):
        """发送夹爪命令"""
        if pos >= 0.5:
            self.ros_node.gripper_pos = 1.0
            self.curr_gripper_pos = np.array([1.0])
            print("Gripper opened")
        elif pos <= -0.5:
            self.ros_node.gripper_pos = 0.0
...
class GripperPenaltyWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.last_gripper_pos = obs["state"]["gripper_pose"][0]
        return obs, info
    ...
    def step(self, action):
        ...
        self.last_gripper_pos = observation["state"]["gripper_pose"][0]
